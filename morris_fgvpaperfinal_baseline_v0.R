# =========================================================
# Morris (EE) — sensitivity for beta0..beta3 and ro
# Baseline embedded in run_model(); saves under .../Graficos FGV final/baseline
# =========================================================
...
suppressPackageStartupMessages({
  ...
  library(tidyr)
  library(tibble)
  library(scales)   # for scales::squish in heatmap
})
...
# =========================================================
# Model function — parameters: beta0..beta3 and ro (rho)
# Others fixed at baseline values
# =========================================================
run_model <- function(beta0, beta1, beta2, beta3, ro) {
  ...
  # Fixed at baseline
  phi0 <- 0.01; phi1 <- 0.05; phi2 <- 0.02; phi3 <- 0.01
  sigma <- 0.50
  del   <- 0
  tau1  <- 0.32
  tau2  <- 0.46
  ...
  rho <- ro  # rename to match equation
  ...
  KY_ratio <- InDebtY_ratio <- EY_ratio <- numeric(t)
  ...
  for (i in 2:t) {
    ...
    denom <- safe_div(Yt[i-1], Ex[i-1])
    OSIN[i] <- ExDebtRes[i-1] / denom
    osin[i] <- if (is.finite(OSIN[i-1]) && OSIN[i-1] != 0) (OSIN[i] - OSIN[i-1]) / OSIN[i-1] else 0

    # (M) Output, capital, and interest
    Yt[i]      <- (1 + ye[i])  * Yt[i-1]
    K[i]       <- (1 + ydt[i]) * K[i-1]
...
# =========================================================
# (2) Factors (Morris) — beta0..beta3 and ro
# =========================================================
factors <- c("beta0", "beta1", "beta2", "beta3", "ro")
binf    <- c(0.05,   1.00,   0.005,  0.01,  0.30)
bsup    <- c(1.20,  10.00,   0.015,  0.10,  0.70)
...
for (j in seq_along(outputs)) {
  ...
  if (any(!is.finite(yj))) {
    fin <- is.finite(yj)
    yj[!fin] <- if (any(fin)) median(yj[fin]) else 0
  }
  mo <- morris_design
  mo <- tell(mo, yj)
  EE <- mo$ee
...
# =========================================================
# (6) Plots — titles removed; axes/legends in English
# =========================================================
...
# (C) EE distributions (violins + box)
p_violins <- ee_long |>
...
# =========================================================
# Save — Desktop/"Graficos FGV final/baseline"
# =========================================================
...
SCEN_NAME <- "baseline"
OUT_DIR   <- file.path(get_desktop_path(), "Graficos FGV final", SCEN_NAME)
